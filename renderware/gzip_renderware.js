...
class GzipRenderware {
  constructor() {
    this.exec = (controller, data, callback) => {
      const contentType = controller.getHeader('Content-Type', '').split(';')[0];
      const acceptEncoding = controller._requestHeaders['accept-encoding'] || '';
      const canCompress = !!{
        'text/plain': 1,
        'text/html': 1,
        'text/xml': 1,
